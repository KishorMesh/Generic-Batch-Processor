...
namespace TaskExecuter.Shared.Actors
{
    /// <summary>
    ...
    public class CommanderActor : ReceiveActor
    {
        ...
        {            
            ...
            {
                _logger.Debug("Commander has received UnableToAcceptJobMessage from {0} for Task ID: {1}", Sender.Path.Name, job.ID);
            ...
            {
                _logger.Debug("Commander has received AbleToAcceptJobMessage from {0} for Task ID: {1}", Sender.Path.Name, job.ID);
